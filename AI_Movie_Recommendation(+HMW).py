...
def suprise(mov=None, genre=None, rating=None, overview=None):
    polarity = TextBlob(str(overview)).sentiment.polarity
    sentiment = f"{Fore.GREEN}Positive 😊" if polarity > 0 else f"{Fore.RED}Negative 😞" if polarity < 0 else f"{Fore.YELLOW}Neutral 😐"
    print (f" {Fore.GREEN}{mov} \n{Fore.CYAN}{rating} {Fore.BLUE}\nMood: , {sentiment}")
...
def handle_ai(name):
    ...
    choice = input("Would you like to customize movie recommandation (type recommendation) or be suprise (type suprise)")
    if choice == "recommendation".lower():
    ...
    elif choice == "suprise".lower():
        # suprise(pd.DataFrame.sample(movies_df))
        suprise(mov=pd.DataFrame.sample(movies_df)["Series_Title"], genre=pd.DataFrame.sample(movies_df)["Genre"], rating=pd.DataFrame.sample(movies_df)["IMDB_Rating"], overview=pd.DataFrame.sample(movies_df)["Overview"])
    else:
